...
1. Write the **minimum** production code needed to pass the current failing test. Dont implement methods of a class implementing an interface that is not needed to pass the test. If you need to implement something in prod, just make the signature to pass and use todos or nulls or whatever you want that is the minimum thing to compile, but dont implement in prod code that has no test to force you to implement it.
...
7. Show the diff of the current production code to the customer
